...
fig, ax = plt.subplots(2, 1, figsize=(ts.FW, 5.9))
...
for s, f in zip(seps, cf):
    a.annotate(f"{s}°", (s, f), textcoords="offset points",
               xytext=(-14, 7) if s == 30 else ((2, 9) if s < 30 else (6, -12)), fontsize=ts.FS, color=K)
...
a.hist(real["32"]["ang"], bins=bins, density=True, histtype="step", color=O, lw=1.8,
       label="traced 32 hpf")
a.set_ylim(0, a.get_ylim()[1] * 1.42)          # headroom so the legend never sits on a bar
a.axvline(60, color="k", ls=":", lw=1)
ymax = a.get_ylim()[1]
a.annotate("60° threshold", (60, ymax * 0.775), textcoords="offset points",
           xytext=(-6, 0), fontsize=ts.FS, color="k", ha="right", va="center")
a.set_xlabel("crossing angle (°)")
a.set_ylabel("density")
ts.panel_title(a, "b  Bimodal: skeletonisation spurs fill the low mode, which is what makes the median unusable", width=64)
a.legend(loc="upper center", ncol=3, columnspacing=1.1, handlelength=1.4)

fig.tight_layout(h_pad=1.6)
fig.savefig("crossing_check_v2.png")
fig.savefig("crossing_check_v2.pdf")
ts.audit(fig, "crossing_check_v2")
